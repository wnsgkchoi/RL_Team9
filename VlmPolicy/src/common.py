...
def save_debug_info(frames, actions, text_descriptions, start, iteration, file_debug):
    for ix, (f, a, t) in enumerate(zip(frames, actions, text_descriptions)):
        file_debug.write(f'{ix+start} - action: {a}, description {t}\n')
        fi = f.permute(2, 0, 1)
        fi = fi.float() / 255.0
        pil_image = transforms.ToPILImage()(fi)
        pil_image.save(f"runs_directory/frame_iter_{iteration}_step_{ix+start}.jpg")
