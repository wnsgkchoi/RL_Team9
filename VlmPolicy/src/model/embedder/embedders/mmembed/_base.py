...
class MMEmbed(BaseEmbedder):
    """
    Base class for MMEmbed model implementations.
    ...
    def __init__(self, **kwargs):
        ...
        self._model_name = "mmembed"
        self._load_cfgs()

        return


class MMEmbed_Factory:
    """
    Factory class for creating MMEmbed model instances based on load mode.

    Usage:
        factory = MMEmbedFactory()
        model = factory.create(mode=LoadMode.LOCAL, **kwargs)
    """

    # Class-level registry shared across factory instances
    _registry: Dict[LoadMode, Type[MMEmbed]] = {}

    def __init__(self) -> None:
        # Instance initializer retained for potential future state
        self._instance_name: str = "MMEmbedFactory"

    @classmethod
    def register(cls, mode: LoadMode, implementation: Type[MMEmbed]) -> None:
        """
    ...
    def create(self, mode: LoadMode, **kwargs) -> MMEmbed:
        """
        Create an MMEmbed instance based on the specified load mode.
        ...
        Returns:
            An instance of the appropriate MMEmbed implementation
