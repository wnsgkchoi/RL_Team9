...
logger = logging.getLogger("MMEmbed_Server")
...
class MMEmbedServer:
    """FastAPI server for MM-Embed model encoding.
    ...
    def __init__(
        ...
        self._app = FastAPI(title="MM-Embed Encoding Server")
        self._model: MMEmbedLocal = None
        ...
        def initiate() -> Dict[str, Any]:
            ...
            try:
                if self._model is None:
                    logger.info("Initializing MMEmbedLocal model...")
                    self._model = MMEmbedLocal(cuda_num=self._cuda_num)
...
if __name__ == "__main__":
    ...
    server = MMEmbedServer(cuda_num=0)
    server.run()
