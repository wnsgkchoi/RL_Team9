...
class NVEmbedV2Client(NVEmbedV2):
    """Client for NV-Embed-v2 remote encoding server.

    This class provides the same API as NVEmbedV2Local but sends requests
    ...
    def __init__(self) -> None:
        ...
        Args:
            host: Server URL (e.g., "http://localhost:13457").
    ...
    def initiate(self) -> dict:
        ...
        try:
            ...
            print()
            print("[NVEmbedV2Client] Connected to remote server successfully.")
    ...
    def free(self) -> None:
        ...
        print()
        print("[NVEmbedV2Client] Model lifecycle managed by remote server.")
        print()
        return
    ...
    def encode_file_offline(
        ...
        if world_size > 1:
            ...
            print(f"[NVEmbedV2Client] Rank {rank}/{world_size}: Processing items {start_idx} to {end_idx-1} (total: {end_idx-start_idx}/{total_items})")
        ...
        print(f"[NVEmbedV2Client] Saved embeddings to {out_embeddings_path} (shape={encoding_output.tensors.shape})")

        # Save index
        IOUtils.write_json_file(encoding_output.index, out_index_path)
        print(f"[NVEmbedV2Client] Saved index to {out_index_path}")

        # Save metadata
        if out_metadata_path:
            IOUtils.write_json_file(enc_metadata.__dict__, out_metadata_path)
            print(f"[NVEmbedV2Client] Saved metadata to {out_metadata_path}")
...
if __name__ == "__main__":
    ...
    NVEmbedV2_Factory.register(LoadMode.REMOTE, NVEmbedV2Client)

    # Create factory and instantiate client
    factory = NVEmbedV2_Factory()
    nvembed = factory.create(mode=LoadMode.REMOTE)

    # Initialize remote model
    model_info = nvembed.initiate()
