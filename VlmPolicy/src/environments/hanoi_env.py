...
from environments.utils import describe_act, HistoryTracker, StatsRecorder
...
class HanoiActions(Enum):
    top_0_to_top_1 = "Move the top disk of rod A to the top of rod B"
    top_0_to_top_2 = "Move the top disk of rod A to the top of rod C"
    top_1_to_top_0 = "Move the top disk of rod B to the top of rod A"
    top_1_to_top_2 = "Move the top disk of rod B to the top of rod C"
    top_2_to_top_0 = "Move the top disk of rod C to the top of rod A"
    top_2_to_top_1 = "Move the top disk of rod C to the top of rod B"

class HanoiEnv(gymnasium.Env):
    default_iter = 10
    default_steps = 30
    ...
    def __init__(self, max_steps=5, num_disks=4, env_noise=0):
        ...
        self.observation_space = spaces.Discrete(1) #spaces.Tuple(self.num_disks*(spaces.Discrete(3),))

        self.current_state = None
        self.goal_state = self.num_disks*(2,)
        self.history = HistoryTracker(max_steps)

        self.action_enum = HanoiActions
...
""".format(list(range(num_disks))[::-1],list(range(num_disks))[::-1], describe_act(self.action_list)).strip()
    ...
    def step(self, action):
        ...
        * Outputs:
        ...
        return 0, reward, self.done, False, info
        #we return always 0 as state, since we really need only info with the textual descriptions, and returning self.current_state breaks the parallel env stacking.
        #return self.current_state, reward, self.done, False, info
    ...
    def reset(self, seed=None, options=None):
        ...
        info["completed"] = 0
        self.history.step(info)
        return 0, info # self.current_state, info
...
def make_hanoi_env(
    outdir,
    env_name,  #Hanoi3Disk-v0, Hanoi4Disk-v0
    save_stats=False
):
    def hanoi_thunk():
        hanoi_env = gymnasium.make(env_name)
        hanoi_env = StepAPICompatibility(hanoi_env, output_truncation_bool=True)
        if save_stats:
            hanoi_env = StatsRecorder(hanoi_env, stat_folder=outdir)
        print(hanoi_env)
        return hanoi_env
