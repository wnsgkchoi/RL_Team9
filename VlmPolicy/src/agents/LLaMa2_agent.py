...
class LLaMaAgent(nn.Module, BaseAgent):
    def __init__(
        self, checkpoint_name, action_enum, is_lora=False, padding_side="right", gradient_ckpt=False, disable_adapters_for_generation=True
    ):
        super().__init__()
    ...
    def generate_continuation(self, prompt):
        ...
        if self.disable_adapters_for_generation and self.is_lora:
            with self.network.disable_adapter():
                generated_ids = self.network.generate(**model_inputs, max_new_tokens=160, pad_token_id=self.tokenizer.eos_token_id)
        else:
            generated_ids = self.network.generate(**model_inputs, max_new_tokens=160, pad_token_id=self.tokenizer.eos_token_id)
    ...
    def create_prompt_for_action_and_value(self, just_value=False, value_prompt_template='{}', action_template='{}', text_description=None, generate_actions=False, advanced_action_matching=False):
        ...
        return prompts, just_context_prompts, match_found, actions_word_count
    ...
    def get_action_and_value_one_forward(self, action=None, just_value=False, value_prompt_template='{}', action_template='{}', text_description=None, temperature=1.0, generate_actions=False, normalization_by_words=False, advanced_action_matching=False):
        ...
        # print(f"{batch_dim=}")
        prompts, just_context_prompt, match_found, actions_word_count = self.create_prompt_for_action_and_value(just_value=just_value,
                                                                                            value_prompt_template=value_prompt_template,
                                                                                            action_template=action_template,
                                                                                            text_description=text_description, 
                                                                                            generate_actions=generate_actions,
    ...
    def _load_model(self, checkpoint="meta-llama/Llama-2-7b-chat-hf", is_lora=False, padding_side="right", gradient_ckpt=False):
